...
### 2.1. Гибридный поиск в Qdrant [✅ Done]
...
### 3.1. Интеграция с LM Studio [✅ Done]
...
| Задача | Детали |
|--------|--------|
### 3.2. Генерация ответа (RAG) [✅ Done]
**Файл:** `app/services/generator.py` (новый)

| Задача | Детали |
|--------|--------|
| Системный промпт | Инструкция для модели: отвечать ТОЛЬКО на основе контекста, игнорировать внешние знания |
| Формирование контекста | Склеивание `Top-K` чанков в единый текст с нумерацией `[1]`, `[2]` |
| Стриминг ответа | Использование `stream=True` для посимвольной отдачи ответа клиенту (Server-Sent Events) |

### 3.3. API Endpoint для Q&A [✅ Done]
**Файл:** `app/api/endpoints.py`

| Задача | Детали |
|--------|--------|
| `POST /ask` | Эндпоинт, объединяющий Retriever, Reranker и Generator |
| Request Schema | `AskRequest`: `query`, `user_roles`, `stream=True/False` |
| Response | Возврат `text/event-stream` при `stream=True` (использовать `fastapi.responses.StreamingResponse`), иначе JSON |
